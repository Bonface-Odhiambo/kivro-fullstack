// African Region Detection for KIVRO Address Generation
// Comprehensive GPS-based region detection for all 54 African countries
...
// Major African cities and regions with GPS coordinates
 ...
 */
export const detectRegionFromGPS = (latitude: number, longitude: number): RegionDetectionResult | null => {
  let bestMatch: RegionDetectionResult | null = null;
 ...
 */
export const detectRegionFromPhone = (phoneNumber: string, latitude?: number, longitude?: number): RegionDetectionResult | null => {
  const phoneValidation = detectCountryFromPhone(phoneNumber);
  
  if (!phoneValidation.isValid || !phoneValidation.countryCode) {
    return null;
  }

  const countryCode = phoneValidation.countryCode.replace('+', '');
  ...
  const isoCode = countryMapping[countryCode];
  ...
  if (latitude && longitude) {
    let bestRegion: RegionDetectionResult | null = null;
    let minDistance = Infinity;
 ...
 */
export const getCitiesForRegion = (countryCode: string, regionName: string): string[] => {
  const countryData = AFRICAN_REGIONS[countryCode];
...
function findClosestCity(lat: number, lng: number, cities: string[], regionData: any): string {
