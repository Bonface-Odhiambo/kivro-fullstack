-- Add support for courier user type and company information (SAFE VERSION)
-- This version handles existing policies and missing functions gracefully
...
-- Create trigger for updated_at on courier_profiles (only if function exists)
DO $$
BEGIN
    IF EXISTS (
    ...
    ELSE
        RAISE NOTICE 'Function handle_updated_at does not exist. Skipping trigger creation.';
    END IF;
END $$;
...
-- Create a view for courier statistics
...
DO $$
BEGIN
    RAISE NOTICE '✅ Courier support migration completed successfully!';
END $$;
